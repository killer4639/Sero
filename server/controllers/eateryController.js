...
const createEateryReview = asyncHandler(async (req, res) => {
  ...
  if (eatery) {
    ...
    eatery.rating = Number(
      (Number(eatery.rating) * Number(eatery.numReviews) + Number(rating)) /
        (eatery.numReviews + 1)
    );
...
const eateryReviews = asyncHandler(async (req, res) => {
  ...
  if (req.user.userType === 1) {
    ...
    const reviewInfo = reviews.map(
      asyncHandler(async (review) => {
    ...
    const eateryUser = await User.find({ eatery: eateryid }).select(
      "-password"
    );
    ...
    if (
      reviews &&
      (req.user._id.toString() == eateryUserId.toString() ||
        req.user.userType === 1)
    ) {
      res.json(reviews);
    } else {
...
const updateEatery = asyncHandler(async (req, res) => {
  console.log("eateary update working");
  const {
    name,
    address,
    description,
